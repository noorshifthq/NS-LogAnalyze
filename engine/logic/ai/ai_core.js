...
export async function initializeLLMEngine() {
    ...
    try {
        ...
        let errorMessage = `Error loading LLM engine.\n\n`;
        ...
        if (ui.activeLlmDisplayRef) { ui.activeLlmDisplayRef.textContent = `Model loading failed for ${config.primaryModelId}`; }
        if (ui.modelProgressContainerRef) ui.modelProgressContainerRef.style.display = 'none';
        if (ui.setAnalyzeButtonStateCallback) ui.setAnalyzeButtonStateCallback(false, primaryError.message || primaryError);
        return false;
        // }
    }
    state.isLLMEngineLoading = false;
